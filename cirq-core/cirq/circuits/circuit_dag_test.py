...
def test_wrapper_repr() -> None:
    ...
    expected = f'cirq.Unique({id(node)}, cirq.X(cirq.LineQubit(0)))'
...
def test_findall_nodes_until_blocked(circuit, is_blocker) -> None:
    ...
    blocked_nodes = blocking_nodes.union(
        *(networkx.descendants(dag, node) for node in blocking_nodes)
    )
    expected_nodes = set(all_nodes) - blocked_nodes
    assert sorted(found_nodes) == sorted(expected_nodes)


def test_classical_controls_and_measurements() -> None:
    q0, q1 = cirq.LineQubit.range(2)
